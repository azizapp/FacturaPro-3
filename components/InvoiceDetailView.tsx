    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="text-2xl font-black text-slate-800 tracking-tight flex items-center space-x-3">
              <span>{invoice.number}</span>
              <span className={`text-[10px] font-black uppercase px-4 py-1.5 rounded-xl border-b-2 ${getStatusStyle(invoice.status)}`}>
                {invoice.status}
              </span>
            </h3>
            <p className="text-xs text-slate-500 font-medium italic mt-1">Émise le {new Date(invoice.date).toLocaleDateString('fr-FR')}</p>
          </div>
        </div>
        <div className="flex space-x-3">
          <button 
            onClick={() => onPdf(invoice.id)}
            className="px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold uppercase text-slate-600 hover:bg-slate-50 shadow-sm flex items-center space-x-2 transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-[12px] p-10 shadow-sm border border-slate-200">
            <div className="grid grid-cols-2 gap-12 mb-12">
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <p className="font-black text-slate-800 uppercase">{company.name}</p>
                  <p className="text-sm text-slate-500 leading-relaxed">{company.address}{company.city ? `, ${company.city}` : ''}</p>
                  <p className="text-sm text-slate-500">SIRET: {company.siret}</p>
                </div>
              </div>
              <div className="space-y-4">
                <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Facturé à</h4>
                <div className="space-y-1">
                  <p className="font-black text-indigo-600 uppercase">{client.name}</p>
                  <p className="text-sm text-slate-500 leading-relaxed">{client.address}</p>
                  <p className="text-sm text-slate-500 leading-relaxed">{client.city}</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-12 p-6 bg-slate-50 rounded-[12px] border border-slate-100">
               ...
               <div>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Date Échéance</p>
                 <p className="text-sm font-black text-rose-500">{new Date(invoice.dueDate).toLocaleDateString('fr-FR')}</p>
               </div>
               <div>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">N° Bon de Commande</p>
                 <p className="text-sm font-black text-indigo-600">{invoice.poNumber || '-'}</p>
               </div>
            </div>
            ...
            <table className="w-full mb-10">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="py-4 text-left text-[10px] font-bold uppercase tracking-widest text-slate-400">Article</th>
                  <th className="py-4 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 w-24">Qté</th>
                  <th className="py-4 text-right text-[10px] font-bold uppercase tracking-widest text-slate-400 w-32">P.U HT</th>
                  <th className="py-4 text-right text-[10px] font-bold uppercase tracking-widest text-slate-400 w-32">
                ...
                {invoice.items.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx}>
                      ...
                      <td className="py-5 text-right font-medium text-slate-500">{item.price.toLocaleString()} MAD</td>
            ...
            <div className="flex flex-col md:flex-row justify-between border-t border-slate-100 pt-8 gap-8">
              <div className="max-w-sm">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Conditions / Notes</p>
                <p className="text-xs text-slate-500 leading-relaxed italic">{invoice.notes || "Aucune condition particulière mentionnée."}</p>
              </div>
              <div className="w-72 space-y-3">
                ...
                {invoice.adjustmentAmount !== 0 && (
                  <div className="flex justify-between text-sm text-slate-500 font-bold italic">
                ...
                <div className="flex justify-between text-lg font-black pt-2 border-t border-slate-200">
                  <span className="text-slate-800">{invoice.taxEnabled ? 'TOTAL TTC' : 'TOTAL'}</span>
                  <span className="text-indigo-600">{invoice.grandTotal.toLocaleString()} MAD</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8">
          <div className="bg-white p-8 rounded-[12px] border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
               <div>
                 <p className="text-2xl font-black text-slate-800">{invoice.grandTotal.toLocaleString()} MAD</p>
               </div>
